...
export default function LoginPopup({ navigate, onClose, handleRegister }) {
  return (
    <div className='fixed top-0 left-0 w-full h-full flex items-center justify-center z-50'>
      ...
      <div aria-label='login popup container'
        className='bg-white flex flex-col
                    ...
                    relative'>
        {/* X to exit */}
        <span className='text-[1.5rem] absolute top-6 right-8 cursor-pointer' onClick={onClose}>
          &times;
        </span>
        {/* Login Form - this form takes you to the homepage if you click 'register' */}
        <LogInForm navigate={navigate} switchForms={handleRegister} />
      </div>
    </div>
  );
}
